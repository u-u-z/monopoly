	...
	var resetTrade = function (initiator, recipient, allowRecipientToBeChanged) {
		...
		for (var i = 0; i < 40; i++) {
			...
			if (currentSquare.owner === initiator.index) {
				...
				currentTableCell.style.backgroundImage = "/images/touhou/" + currentSquare.color;
				...
				currentTableCell.className = "propertycellcolor";
				//currentTableCell.style.backgroundColor = currentSquare.color;
				currentTableCell.style.backgroundImage = `url(${"/images/touhou/" + currentSquare.color})`;
...
function updatePosition() {
	...
	for (var x = 0; x < 40; x++) {
		...
		for (var y = turn; y <= pcount; y++) {
			...
			if (player[y].position == x && !player[y].jail) {
				...
				document.getElementById("cell" + x + "positionholder").innerHTML += "<div class='cell-position' title='" + player[y].name + "' style='background-image:url(/image/touhou/" + player[y].color + "); left: " + left + "px; top: " + top + "px;'></div>";
				if (left == 36) {
					left = 0;
					top = 12;
				} else
	...
	for (var i = turn; i <= pcount; i++) {
		if (player[i].jail) {
			document.getElementById("jailpositionholder").innerHTML += "<div class='cell-position' title='" + player[i].name + "' style='background-image:url(/image/touhou/" + player[i].color + "); left: " + left + "px; top: " + top + "px;'></div>";
...
function updateOwned() {
	...
	for (var i = 0; i < 40; i++) {
		...
		if (sq.groupNumber && sq.owner === 0) {
			...
			currentTableCell.style.backgroundImage = "url(/image/touhou/" + player[sq.owner].color + ")";
...
function showdeed(property) {
	...
	if (sq.mortgage) {
		...
		if (sq.groupNumber >= 3) {
			$("#deed-normal").show();
			document.getElementById("deed-header").style.backgroundImage = "url(/image/touhou/" + sq.color + ")";
...
window.onload = function () {
	...
	for (var i = 0; i < 40; i++) {
		...
		document.getElementById("enlarge" + i + "color").style.backgroundImage = "url(/image/touhou/" + s.color + ")";
